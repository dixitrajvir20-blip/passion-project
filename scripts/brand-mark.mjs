 ...
 * B cut from Bricolage Grotesque 800 (a path, so no font is needed). Replace both with the licensed
 * logo when the file arrives (docs/BRAND_GUIDE.md §4), then run `node scripts/brand-icons.mjs`.
 *
 * Two versions of the same drawing, nothing recoloured: badge() is the default (blue disc, white
 * ring and B) for the favicon and icons; badgeInverse() swaps the two colours (white disc, blue ring
 * and B) for anything that sits on the blue bar, such as the top of the share images.
 */
...
const draw = (disc, mark) =>
  `<circle cx="32" cy="32" r="32" fill="${disc}"/>` +
  `<circle cx="32" cy="32" r="27.5" fill="none" stroke="${mark}" stroke-width="1.5"/>` +
  `<path fill="${mark}" transform="translate(16.5 47) scale(0.04545 -0.04545)" d="${B_PATH}"/>`;

/** The badge drawn in a 64×64 box, as markup ready to be translated and scaled. */
export const badge = () => draw(BLUE, WHITE);

/** The inverse badge, for the blue bar: a white disc with a blue ring and B. */
export const badgeInverse = () => draw(WHITE, BLUE);
