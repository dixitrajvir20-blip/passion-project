/** Foundation stub: the buffer-target builder replaces this file with the spec's e2e cases. */
...
const ready = toolBySlug('buffer-target')!.ready !== false;

for (const edition of ['in', 'eu', 'us']) {
  test(`${edition}/tools/buffer-target opens`, async ({ page }) => {
    const response = await page.goto(`${edition}/tools/buffer-target`);
    if (!ready) {
      expect(response?.status()).toBe(404);
      return;
    }
    await waitForIslands(page);
    await expect(page.locator('h1')).toHaveText('Buffer target and the payday it is reached');
