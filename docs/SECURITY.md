...
| 10 | Open redirect / reverse tabnabbing; figures leaking through a shared link | M | M | No redirect from URL/localStorage values; every external link `rel="noopener noreferrer"`. Calculator links carry inputs in the `#` fragment, which browsers never send to a server; the fragment is read once and removed from the address bar (old `?query` links still open, and their query is left alone); the page tells the reader that anyone they send it to sees the numbers | Now | Done |
